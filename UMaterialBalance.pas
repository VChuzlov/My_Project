...
type
  ...
  TMatBalance = Class
    ...
    function getTrayBalanceError(Fj, Uj, Wj, Lj, Vj: arrTrays): arrTrays;
...
function TMatBalance.getTrayBalanceError(Fj, Uj, Wj, Lj, Vj: arrTrays): arrTrays;
var
 j: integer;
 input: arrTrays;
 output: arrTrays;
...
begin
  ...
  TrayBalanceError := getTrayBalanceError(Fj, Uj, Wj, Lj0, Vj0);
  ...
  for j := 1 to Ntrays do
    begin
      ...
      for i := 1 to NComp do
        begin
          {
          Это закомментировано, т.к. можно взять индивидуальные Тк
          Для неизвестных компонентов она может быть рассчитана
      ...
      for i := 1 to NComp do
        begin
          ...
          CompItnLiqCp[i-1, j-1] := IntegralLiquidCp({Tsat}Tsat, Tj[j])[i];
        end;
    end;
  for j := 1 to NTrays do
    begin
...
begin
  ...
  repeat
    ...
    TrayBalanceError := getTrayBalanceError(Fj, Uj, Wj, Lj, Vj);
    n := n + 1;
    if n >= 1e5 then
      begin
