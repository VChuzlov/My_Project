...
type
  ...
  TMatBalance = Class
    ...
    procedure CalculateVaporFlowRates(Hf_l, Hf_v, H_l, H_v: arrTrays; Fj, Lj, Uj, Wj: arrTrays;
      Qj: arrTrays; ej: arrTrays; LD, VD, rB: double; var Vj: arrTrays);
    ...
    function global_energy_balance(Fj, ej, Uj, Wj, Qj, Hf_l, Hf_v, H_l, H_v: arrTrays): double;
...
var
  ...
  rD: double; // ôëåãìîâîå ÷èñëî // ïðèíÿë çà 1
  rB: double; // ïàðàâîå ÷èñëî // ïðèíÿë çà 1
  D, B: double;
  s: double;
  ...
  begin
    ...
    for j := 1 to NTrays do
      begin
        ...
        qj[j] := 1; //ïî èäåå íàäî åãî ñ÷èòàòü ÷åðåç ýíòàëüïèþ â çàâèñèìñîòè îò ñîñòîÿíèÿ
      end;
...
begin
  ...
  Uj[1] := LD;
  Wj[1] := WD;
  Uj[NTrays] := Fj[FeedTray] - (LD + WD);  // ä.á. ñóììà Fj
  Vj0[1] := 0; // äîëæíî çàâèñåòü îò òèïà êîíäåíñàòîðà
  Lj0[1] := LD * rD;
...
const
// Ïàðàìåòðû äëÿ ðàñ÷åòà ýíòàëüïèè èäåàëüíîãî ãàçà
...
var
  i, j: integer;
begin
  // Òåìïåðàòóðà äîëæíà áûòü â Êåëüâèíàõ
  for j := 1 to NTrays do
    for i := 1 to NComp do
...
var
  ...
  var
    ...
    function f(rB: double): double;
    var
      global_energy_balance_error: double;
    begin
...
function TMatBalance.global_energy_balance(Fj, ej: arrTrays; Uj: arrTrays; Wj: arrTrays;
  Qj: arrTrays; Hf_l: arrTrays; Hf_v: arrTrays; H_l: arrTrays; H_v: arrTrays): double;
var
  j: integer;
  s: double;
begin
  s := 0;
  for j := 1 to NTrays do
    s:= s + Fj[j] * (Hf_v[j] * ej[j] + Hf_l[j] * (1 - ej[j]));
  for j := 1 to NTrays do
    s := s - (Wj[j] * H_v[j] + Uj[j] * H_l[j] + Qj[j]);
  Result := s;
end;
...
var
  ...
  n: integer;
  omegaj: ArrTrays; // äîëÿ ïàðà â áîêîâûõ îòáîðàõ
  TrayMaterialBalanceError: arrTrays;
  TrayHeatBalanceError: arrTrays;
  rB: double;
  global_energy_balance_error: double;

  procedure Calculation(rB: double; TrayHeatBalanceError: arrTrays);
  var
    j: integer;
  begin
  ...
  const
    ...
    begin
      ...
      Result := TrayMaterialBalanceError[1]
         {+ global_energy_balance(Fj, ej, Uj, Wj, Qj, Hf_l, Hf_v, H_l, H_v)};
    end;
  begin
    if f(a) * f(b) < 0 then
      repeat
...
begin
  ...
  for j := 1 to NTrays do
    begin
      Tfj[j] := 50 + 273.15;
      Pfj[j] := Pj[j];
      omegaj[j] := 0; // Ò.ê. âñå îòáðû â âèäå æèäêîñòè
    end;
  ...
  repeat
    ...
    CalculateVaporFlowRates(Hf_l, Hf_v, H_l, H_v, Fj, Lj0, Uj, Wj, Qj, ej, 4.5, 0, 1, Vj);
    CalculateHeatDuties(Fj, ej, Lj0, Vj, Uj, Wj, Hf_l, Hf_v, H_l, H_v, Qc, Qr);
    CalculateSideFlows_and_LiquidFlows(Fj, omegaj, Vj, Lj0, 4.5, 13.5, Wj, Uj, Lj);
    TrayMaterialBalanceError := getTrayMaterialBalanceError(Fj, Uj, Wj, Lj, Vj);
    TrayHeatBalanceError := getTrayHeatBalanceError(Fj, Uj, Wj, Lj, Vj, ej, Hf_l, Hf_v, H_l, H_v);
    ...
    ej_tr := getTrays_ej(Fj, Lj, Vj, zf, xij, yij, Tj, Pj); // êàê îêàçàëîñü, ýòî íå íóæíî
    n := n + 1;
    if n >= 1e5 then
      begin
